...
function calc() {
  var persons = document.querySelectorAll('.counter-block-input')[0],
      ...
      personsSum = 0,
      // когда пользователь будет записывать количество людей, будем записывать их в эту переменную
  daysSum = 0,
      // будем записывать количество дней
  ...
  place.addEventListener('change', function () {
    if (restDays.value == '' || persons.value == '') {
      ...
      var a = total; // создаем промежуточную переменную, чтобы постоянно не переопределять переменную total
...
function form() {
  ...
  var message = {
    loading: 'Загрузка...',
    // Эта строка будет показываться, когда наш запрос еще не обработался
  ...
  statusMessage.classList.add('status'); // Для того, чтобы отправить форму, нужно, чтобы было button, либо <input type="sumbit">

  function sendForm(elem) {
    // создаем функцию
    elem.addEventListener('submit', function (e) {
      // обработчик событий на submit каждой формы, а НЕ КНОПКИ!!!
      ...
      var formData = new FormData(elem); // каждый раз создается именно та formData, откуда получаем данные, вместо elem указанная форма через аргумент

      function postData(data) {
        // в аргумент подставляется formData
        return new _Promise(function (resolve, reject) {
          // возвращаем сразу новый промис без функции
          ...
          request.onreadystatechange = function () {
            // обработчий события readystatechange, каждый раз проверяет изменения текущего статуса запроса
      ...
      function clearInput() {
        for (var i = 0; i < input.length; i++) {
          // после сабмита очищаем все наши инпуты
          input[i].value = '';
        }
      }
...
function popup() {
  var more = document.querySelector('.more'),
      ...
      descriptionBtn = document.querySelectorAll('.description-btn'); // Мой вариант:
  ...
  descriptionBtn.forEach(function (descriptionBtn) {
    // перебираю созданный массив аргументом является целый массив
  ...
  close.addEventListener('click', function () {
    // здесь можно использовать стрелочные функции, но хз зачем
  ...
  overlay.addEventListener('click', function (event) {
    // функция, реализующая закрытие попапа при клике на overlay
    if (event.target == overlay) {
      // проверка на то что, именно когда мы тыкаем по overlay закрывалось модальное окно
      for (var i = 0; i < descriptionBtn.length; i++) {
        descriptionBtn[i].classList.remove('more-splash');
      }
...
function slider() {
  var slideIndex = 1,
      // параметр текущего слайда, именно его мы и будем менять
  slides = document.querySelectorAll('.slider-item'),
      // получаем псевдомассив со всеми слайдами
  prev = document.querySelector('.prev'),
      // правая стрелочка навигации
  next = document.querySelector('.next'),
      // левая стрелочка навигации
  dotsWrap = document.querySelector('.slider-dots'),
      // обертка переключателей
  ...
  function showSlides(n) {
    // аргумент n, для того чтобы когда мы будем вызывать эту функцию, она нам переключала слайды
    if (n > slides.length) {
      // проверка: если наши слайды закончились в каруселе, то мы возвращаемся к первому
      slideIndex = 1;
    }

    if (n < 1) {
      // если n < 1, то мы возвращаемся к самому последнему слайдеру
      slideIndex = slides.length; // индекс + 1 в length
    }

    slides.forEach(function (item) {
      return item.style.display = 'none';
    }); // перебрали и скрыли все слайды
    ...
    }); // убрали у всех точек активный класс
  ...
  function plusSlides(n) {
    // сюда мы например подставляем 1
  ...
  function currentSlide(n) {
    // когда мы тыкаем на четвертую точку например
...
function smoothScroll() {
  ...
  menu.addEventListener('click', function (event) {
    ...
    var target = event.target,
        // создали переменную в которой будет кликнутая ссылка
    blockID = target.getAttribute('href'),
        // эта переменная, в этой кликнутей ссылке будет помещен атрибут ссылка
...
function tabs(tabcontents, wrapper, tabs) {
  var tabContent = document.getElementsByClassName(tabcontents),
      // Псевдомассив со всеми таб контентами от 0 до 4
  ...
  function hideTabContent(a) {
    // a - технический аргумент, он будет 1, он подставляется в i и скроется весь таб контент кроме первого таб контента
    for (var i = a; i < tabContent.length; i++) {
      // tabContent.lenght это количество элементов в псевдомассиве tabContent
  ...
  function showTabContent(b) {
    // b - технический аргумент, он нам нужен для того чтобы показать именно тот таб контент, который нам необходим при клике на таб
    if (tabContent[b].classList.contains('hide')) {
      // проверка действительно ли какой-то таб контент в зависимости от i скрыт? если скрыт, то:
  ...
  info.addEventListener('click', function (event) {
    // объект event, ведь мы будем сравнивать куда мы кликаем
    var target = event.target; // помещаем в переменную на что именно происходит клик

    if (target && target.classList.contains(tabs)) {
      // проверка на то, что таргет вообще есть и то что мы тыкнули именно на элемент с классом info-header-tab
      // 4) Здесь нам необходимо определить что каждый таб связан со своим табконтентом
      for (var i = 0; i < tab.length; i++) {
        // запускаем цикл, который перебирает табы и сравнивает каждый таб с тем куда мы кликнули
        if (target == tab[i]) {
          // если то куда мы нажали полностью совпадает с тем табом куда мы нажали, то произойдет:
...
function timer() {
  ...
  // 1) Задать дедлайн, время до которого наш таймер будет отсчитывать:
  var deadline = '2019-06-02'; // 2) Получить разницу между дедлайном и текущем временем и из этого значения вытащить секунды, минуты, часы:

  function getTimeRemaining(endtime) {
    // функция, которая будет считать разницу между дедлайном и текущем временем
    ...
    seconds = Math.floor(t / 1000 % 60),
        // Math.floor - получаем только целые числа; % 60 - это чтобы получать хвостик-остаток от целого и чтобы число не было больше 60
    ...
    return {
      // говорим функции верни нам это
      // так как мы не можем вернуть из функции сразу несколько функций
      'total': t,
      // в будущемы будем использовать эту переменную как таймер, как только только она станет 0 мы остановим функцию
      'hours': hours,
      'minutes': minutes,
      'seconds': seconds
    };
  ...
  function setClock(id, endtime) {
    // аргумент id - это айди родителя всех спанов со значениями, то есть айди обертки, куда мы будем вставлять все значения
    var timer = document.getElementById(id),
        // указанный id обертки
    hours = timer.querySelector('.hours'),
        // querySelector получает первый элемент с таким классом на странице
    ...
    function updateClock() {
      var t = getTimeRemaining(endtime); // заточаем в переменную t, тот объект который мы вернули из функции пунктом 2)
      ...
      if (t.total <= 0) {
        // как только total будет равно 0 или меньше все останавливай
...
window.addEventListener('DOMContentLoaded', function () {
  // Скрипт будет выполняться только после того, как прогрузиться DOM дерево (HTML)
  'use strict'; // Активен строгий режим

  var calc = __webpack_require__(/*! ./parts/calc.js */ "./js/parts/calc.js"),
      // ./ это то что находится в этой же папке
  form = __webpack_require__(/*! ./parts/form.js */ "./js/parts/form.js"),
      ...
      popup = __webpack_require__(/*! ./parts/popup.js */ "./js/parts/popup.js"),
      smooth = __webpack_require__(/*! ./parts/smoothscroll.js */ "./js/parts/smoothscroll.js");
